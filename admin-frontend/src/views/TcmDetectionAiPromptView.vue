...
import { fetchTcmDetectionAiPrompts, updateTcmDetectionAiPrompt } from '../api/admin'
import type { TcmDetectionAiPromptTemplate } from '../types/admin'

const loading = ref(false)
const saving = ref(false)
const rows = ref<TcmDetectionAiPromptTemplate[]>([])
...
async function loadRows() {
  loading.value = true
  try {
    rows.value = await fetchTcmDetectionAiPrompts()
  } catch (e) {
...
function openEdit(row: TcmDetectionAiPromptTemplate) {
...
async function submit() {
  ...
  saving.value = true
  try {
    await updateTcmDetectionAiPrompt(currentId, {
...
<template>
  <div class="tcm-prompt-page">
    <el-card class="page-intro" shadow="never">
      <div class="page-intro__title">中医体检 AI 提示词配置</div>
      <div class="page-intro__desc">维护综合报告和九种体质分项的 DeepSeek 提示词模板。模板支持变量：{{ '{' }}{{ '{' }}userProfileText{{ '}' }}{{ '}' }}、{{ '{' }}{{ '{' }}itemsFactsText{{ '}' }}{{ '}' }}、{{ '{' }}{{ '{' }}itemFactsText{{ '}' }}{{ '}' }}。</div>
    </el-card>
...
<style scoped>
.tcm-prompt-page { display: grid; gap: 16px; }
