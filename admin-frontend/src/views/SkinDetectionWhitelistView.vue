...
import {
  adjustSkinDetectionQuota,
  createSkinDetectionWhitelist,
  fetchSkinDetectionConsumeLogs,
  fetchSkinDetectionManualLogs,
  fetchSkinDetectionQuotaSummary,
  fetchSkinDetectionWhitelist,
  updateSkinDetectionWhitelist,
} from '../api/admin'
import type { SkinDetectionQuotaLogItem, SkinDetectionQuotaSummary, SkinDetectionWhitelist } from '../types/admin'

const loading = ref(false)
const saving = ref(false)
const quotaSaving = ref(false)
const detailLoading = ref(false)
const rows = ref<SkinDetectionWhitelist[]>([])
const keyword = ref('')
...
const currentRow = ref<SkinDetectionWhitelist | null>(null)
const formRef = ref<FormInstance>()
const quotaFormRef = ref<FormInstance>()

const form = reactive<SkinDetectionWhitelist>({
  phone: '',
  remark: '',
  status: 1,
  totalTimes: 3,
...
const quotaSummary = ref<SkinDetectionQuotaSummary | null>(null)
const manualLogs = ref<SkinDetectionQuotaLogItem[]>([])
const consumeLogs = ref<SkinDetectionQuotaLogItem[]>([])
...
async function loadRows() {
  loading.value = true
  try {
    rows.value = await fetchSkinDetectionWhitelist({
...
function openEdit(row: SkinDetectionWhitelist) {
...
function openQuotaAdjust(row: SkinDetectionWhitelist) {
...
async function openBalanceDetail(row: SkinDetectionWhitelist) {
  if (!row.id) return
  currentRow.value = row
  ...
  try {
    const [summary, logs] = await Promise.all([
      fetchSkinDetectionQuotaSummary(row.id),
      fetchSkinDetectionManualLogs(row.id),
    ])
...
async function openConsumeDetail(row?: SkinDetectionWhitelist) {
  ...
  detailLoading.value = true
  try {
    consumeLogs.value = await fetchSkinDetectionConsumeLogs(target.id)
  } catch (e) {
...
async function submitForm() {
  ...
  try {
    ...
    if (editingId.value) {
      await updateSkinDetectionWhitelist(editingId.value, payload)
      ElMessage.success('已更新')
    } else {
      await createSkinDetectionWhitelist(payload)
...
async function submitQuotaAdjust() {
  ...
  try {
    await adjustSkinDetectionQuota(currentRow.value.id, {
...
function getBalance(row: SkinDetectionWhitelist) {
...
<template>
  <div class="skin-whitelist-page">
    ...
    <el-card>
      ...
      <el-alert title="新增手机号默认 3 次；只有绑定该手机号且白名单启用、剩余次数大于 0 的用户，才能提交皮肤检测。" type="info" show-icon :closable="false" class="member-tip" />
    ...
    <el-dialog v-model="quotaDialogVisible" title="调整皮肤检测次数" width="460px">
...
<style scoped>
.skin-whitelist-page { display: grid; gap: 16px; }
