...
import { fetchSkinDetectionItemConfigs, updateSkinDetectionItemConfig } from '../api/admin'
import type { SkinDetectionItemConfig } from '../types/admin'

const loading = ref(false)
const saving = ref(false)
const rows = ref<SkinDetectionItemConfig[]>([])
...
async function loadRows() {
  loading.value = true
  try {
    rows.value = await fetchSkinDetectionItemConfigs()
  } catch (e) {
    ElMessage.error(e instanceof Error ? e.message : '加载检测项配置失败')
  } finally {
    loading.value = false
  }
}

function openEdit(row: SkinDetectionItemConfig) {
  editingId.value = row.id
  form.itemCode = row.itemCode
  form.yimeiField = row.yimeiField
  form.detectType = row.detectType
...
async function submit() {
  ...
  saving.value = true
  try {
    await updateSkinDetectionItemConfig(currentId, {
...
<template>
  <div class="skin-config-page">
    <el-card class="page-intro" shadow="never">
      <div class="page-intro__title">皮肤检测项配置</div>
      <div class="page-intro__desc">维护皮肤检测项的启用状态、展示名称、排序、分档类型和对应 AI 提示词。itemCode、宜远字段和 detectType 为关键接口字段，不在此页面修改。</div>
    </el-card>
    ...
    <el-card>
      ...
      <el-table :data="rows" v-loading="loading" border>
        ...
        <el-table-column prop="yimeiField" label="宜远字段" width="130" />
        <el-table-column prop="detectType" label="detectType" width="120" />
    ...
    <el-dialog v-model="dialogVisible" title="编辑检测项配置" width="640px" destroy-on-close>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="120px">
        <el-form-item label="itemCode">
          <el-input v-model="form.itemCode" disabled />
        </el-form-item>
        <el-form-item label="宜远字段">
          <el-input v-model="form.yimeiField" disabled />
        </el-form-item>
        <el-form-item label="detectType">
          <el-input-number v-model="form.detectType" disabled :controls="false" style="width: 100%" />
        </el-form-item>
        ...
        <el-form-item label="分档类型" prop="scaleType">
          <el-select v-model="form.scaleType" style="width: 100%">
            ...
            <el-option label="肤质 skin_type" value="skin_type" />
          </el-select>
        </el-form-item>
...
<style scoped>
.skin-config-page { display: grid; gap: 16px; }
