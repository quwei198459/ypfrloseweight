  ...
  <view class="camera-page">
    <view class="camera-shell">
      <camera
        ...
        @error="onCameraError"
      />
      <image v-else class="camera-view" :src="currentImage" mode="aspectFill" />

      <!-- 进度指示：舌象 / 面象 -->
      <view class="stage-bar" pointer-events="none">
        <view class="stage-chip" :class="{ 'stage-chip--active': stage === 'tongue', 'stage-chip--done': !!tongueImage }">
          <text class="stage-chip__no">1</text>
          <text class="stage-chip__text">舌象</text>
        </view>
        <view class="stage-divider"></view>
        <view class="stage-chip" :class="{ 'stage-chip--active': stage === 'face', 'stage-chip--done': !!faceImage }">
          <text class="stage-chip__no">2</text>
          <text class="stage-chip__text">面象</text>
        </view>
      </view>

      <!-- 舌象引导：椭圆形（嘴部）轮廓 -->
      <view v-if="stage === 'tongue'" class="tongue-guide" pointer-events="none">
      ...
      <!-- 面象引导：面部网格轮廓 -->
      <view v-else class="face-guide" pointer-events="none">
        <view class="corner corner--lt"></view>
        <view class="corner corner--rt"></view>
        <view class="corner corner--lb"></view>
        <view class="corner corner--rb"></view>
      ...
      <view v-if="!currentImage" class="camera-tip">
        <text class="camera-tip__title">{{ stage === 'tongue' ? '请伸出舌头置于轮廓内' : '请将面部置于轮廓内' }}</text>
        <text class="camera-tip__desc">{{ stage === 'tongue' ? '自然伸舌、光线充足，舌面完整无遮挡' : '保持正脸、光线充足，额头和脸颊无遮挡' }}</text>
      </view>
      <view v-else class="camera-tip camera-tip--preview">
        <text class="camera-tip__title">确认照片是否清晰</text>
        <text class="camera-tip__desc">{{ stage === 'tongue' ? '如舌面不完整或光线较暗，请重新拍摄' : '如脸部不完整或光线较暗，请重新拍摄' }}</text>
      </view>
    </view>

    <view class="bottom-panel">
      <view class="side-action" @click="currentImage ? retake() : goBack()">
        <text class="side-action__text">{{ currentImage ? '重拍' : '取消' }}</text>
      </view>

      <view v-if="!currentImage" class="shutter" @click="takePhoto">
        <view class="shutter__inner"></view>
      </view>
      <view v-else class="use-photo" @click="confirmCurrent">
        <text class="use-photo__text">{{ stage === 'tongue' ? '下一步（面象）' : '使用照片' }}</text>
      </view>

      <view class="side-action side-action--right" @click="currentImage ? undefined : switchCamera()">
        <text v-if="!currentImage" class="side-action__icon">↻</text>
      </view>
    </view>
  </view>
</template>
...
// 采集阶段：先舌象，后面象
const stage = ref<'tongue' | 'face'>('tongue')
const tongueImage = ref('')
const faceImage = ref('')
// 当前阶段拍摄/预览中的临时图片
const previewImage = ref('')
// 舌象、面象均默认前置（自拍模式），用户可用右下角按钮手动切换
const devicePosition = ref<'front' | 'back'>('front')

const currentImage = computed(() => previewImage.value)
...
function takePhoto() {
  ...
  ctx.takePhoto({
    quality: 'high',
    success: (res) => {
      if (res.tempImagePath) previewImage.value = res.tempImagePath
    },
    fail: () => {
...
function confirmCurrent() {
  if (!previewImage.value) {
    uni.showToast({ title: stage.value === 'tongue' ? '请先拍摄舌象照' : '请先拍摄面象照', icon: 'none' })
    return
  }
  if (stage.value === 'tongue') {
    // 保存舌象，进入面象采集
    tongueImage.value = previewImage.value
    uni.setStorageSync(STORAGE_TONGUE_IMAGE, tongueImage.value)
    previewImage.value = ''
    stage.value = 'face'
  ...
  // 面象确认完成，两张齐备，进入生成报告
  faceImage.value = previewImage.value
  uni.setStorageSync(STORAGE_FACE_IMAGE, faceImage.value)
  uni.navigateTo({ url: '/pages/tcm-detection/loading' })
}
...
.stage-chip--active .stage-chip__no {
  background: #9ebc86;
  border-color: #9ebc86;
}

.stage-chip--done .stage-chip__no {
  background: #9ebc86;
  border-color: #9ebc86;
}
...
.tongue-mesh {
  ...
  height: 460rpx;
  transform: translate(-50%, -50%);
  border-radius: 48% 48% 50% 50% / 40% 40% 60% 60%;
